import pytest
from model.application import Application, YesOrNoException
...
@pytest.mark.parametrize("command,expected_method", [
    ...
    ('узнать статус пациента', empty_application.print_patient_status),
...
def test_answer_nonsense(prepared_app):
    ...
    with pytest.raises(YesOrNoException):
        prepared_app._ask_yes_or_no()
...
def test_get_status(prepared_app):
    prepared_app.db_interface.db = [1, 3, 1]
    prepared_app._ask_for_index = lambda: 2 - 1
    status = prepared_app.print_patient_status()
...
def test_get_non_existent_patient_status(prepared_app):
    ...
    prepared_app._ask_for_index = lambda: 100-1
    assert 'Пациент с индексом 100 не найден' == \
           prepared_app.print_patient_status()
