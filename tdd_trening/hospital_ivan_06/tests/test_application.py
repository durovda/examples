import pytest
from model.application import Application, NotIntegerException
from model.db import HospitalListDB, UnknowUserException
...
def test_getting_non_int_for_index(prepared_app):
    ...
    with pytest.raises(NotIntegerException):
...
def test_try_to_raise_nonexistent_patient(prepared_app):
    prepared_app._ask_for_index = lambda: 99
    with pytest.raises(UnknowUserException):
        prepared_app.raise_patient_status()
...
def test_discharge_patient_with_max_status(prepared_app):
    ...
    new_status = prepared_app.raise_patient_status()
    assert new_status == f'Пациент с ID=2 выписан.'
